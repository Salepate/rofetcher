...
function wish(ctx, args)
{
    ...
    if ( args[0] === 'help')
    {
        ctx.response(`syntax: !wish show/add/remove [user] <item_id1> <item_id2>\n\nshow [user] # show user's wishlist (self if none specified)`+ 
        `\nadd <item_id1> <item_id2> ... # register items to your own list\nremove <item_id1> <item_id2> ... # remove items from your own list`, false, true);
    }
    if ( args[0] == 'add')
    {
            ...
            if ( items.length > 0 )
            {
                database.collection("items").find({"id": { "$in" : items}}).toArray((err, res)=>
                {
                    ...
                    for(let i = 0; i < res.length; ++i)
                    {
                        ...
                        response.push(`${itemDesc.display_name} (${itemDesc.id}) : ${wishlist[i].amount}`);
                    }
